...
@pytest.mark.parametrize("email, password, reg_status_code, login_status_code, wrong_login_status_code, me_status_code", [
...
async def test_full_auth(
    ...
    wrong_login_resp1 = await ac.post(
        "/auth/login",
        json={"email": email, "password": "wrong_pass"}
    )
    assert wrong_login_resp1.status_code == wrong_login_status_code

    wrong_login_resp2 = await ac.post(
        "/auth/login",
        json={"email": "wrong_email@mail.com", "password": password}
    )

...
def get_available_rooms_ids_query(date_from: date, date_to: date):
    rooms_count = (
        ...
        .filter(Bookings.date_from <= date_to, Bookings.date_to >= date_from)
    ...
    available_rooms_ids = select(rooms_left_table.c.room_id).filter(rooms_left_table.c.rooms_left > 0)

...
async def get_room(room_id: int):
    async with async_session_maker() as session:
        rooms_repository = RoomsRepository(session)
        room = await rooms_repository.get_one_or_none(id=room_id)
...
async def delete_rooms(room_id: int):
    async with async_session_maker() as session:
        rooms_repository = RoomsRepository(session)
        await rooms_repository.delete(id=room_id)
...
async def create_room(room_data: RoomAdd = Body(openapi_examples={
    ...
    async with async_session_maker() as session:
        rooms_repository = RoomsRepository(session)
        room = await rooms_repository.add(room_data)
        await session.commit()
...
async def patch_room(room_id: int, room_data: RoomPatch):
    async with async_session_maker() as session:
        rooms_repository = RoomsRepository(session)
        await rooms_repository.edit(room_data, exclude_unset=True, id=room_id)
        await session.commit()
...
async def put_room(room_id: int, room_data: RoomAdd):
    async with async_session_maker() as session:
        rooms_repository = RoomsRepository(session)
        await rooms_repository.edit(room_data, id=room_id)
        await session.commit()
